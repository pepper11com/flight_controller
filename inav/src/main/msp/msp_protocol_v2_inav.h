...
// Direct motor override (runtime) for external controllers when MSP RC Override is active
// Payload: sequence of uint16 motor commands in microseconds (mincommand..max_throttle) for motors 1..N
// The provided values are applied for a short timeout window and refreshed on new packets.
#define MSP2_INAV_SET_MOTOR_OVERRIDE            0x20F5

// Direct servo override (runtime) for external controllers when MSP RC Override is active
// Payload: sequence of uint16 servo commands in microseconds for servo outputs 0..N-1 (in write order)
// Values are applied for a short timeout window and refreshed on new packets.
#define MSP2_INAV_SET_SERVO_OVERRIDE            0x20F6
