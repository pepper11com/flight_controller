...
#include "msp/msp_servo_override.h"
...
static mspResult_e mspFcProcessInCommand(uint16_t cmdMSP, sbuf_t *src)
{
    ...
    switch (cmdMSP) {
    case MSP2_INAV_SET_MOTOR_OVERRIDE: {
#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
        const unsigned int countWords = sbufBytesRemaining(src) / (unsigned int)sizeof(uint16_t);
        if (countWords == 0) {
            return MSP_RESULT_ERROR;
        }
        uint16_t tmpBuf[MAX_SUPPORTED_MOTORS];
        const unsigned int n = MIN((unsigned int)MAX_SUPPORTED_MOTORS, countWords);
        for (unsigned int i = 0; i < n; i++) {
            tmpBuf[i] = sbufReadU16(src);
        }
        // drain any extra words if payload longer than supported
        for (unsigned int i = n; i < countWords; i++) {
            (void)sbufReadU16(src);
        }
        mspMotorOverrideUpdate(tmpBuf, (uint8_t)n, micros());
        return MSP_RESULT_ACK;
#else
        UNUSED(src);
        return MSP_RESULT_ERROR;
    ...
    case MSP2_INAV_SET_SERVO_OVERRIDE: {
#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
        const unsigned int countWords = sbufBytesRemaining(src) / (unsigned int)sizeof(uint16_t);
        if (countWords == 0) {
            return MSP_RESULT_ERROR;
        }
        uint16_t tmpBuf[MAX_SUPPORTED_SERVOS];
        const unsigned int n = MIN((unsigned int)MAX_SUPPORTED_SERVOS, countWords);
        for (unsigned int i = 0; i < n; i++) {
            tmpBuf[i] = sbufReadU16(src);
        }
        // drain any extra words
        for (unsigned int i = n; i < countWords; i++) {
            (void)sbufReadU16(src);
        }
        mspServoOverrideUpdate(tmpBuf, (uint8_t)n, micros());
        return MSP_RESULT_ACK;
#else
        UNUSED(src);
        return MSP_RESULT_ERROR;
