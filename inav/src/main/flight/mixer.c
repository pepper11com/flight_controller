...
#include "msp/msp_motor_override.h"
#endif
...
#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
static reversibleMotorsThrottleState_e updateReversibleThrottleStateFromOverride(uint16_t overrideUs);
#endif
...
#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
    const bool overrideModeEnabled = IS_RC_MODE_ACTIVE(BOXMSPRCOVERRIDE) && !failsafeIsActive();
    const timeUs_t overrideCheckTimestamp = overrideModeEnabled ? micros() : 0;
    const bool mspMotorOverrideActive = overrideModeEnabled && mspMotorOverrideIsFresh(overrideCheckTimestamp);
#endif
    for (int i = 0; i < motorCount; i++) {
        uint16_t motorValue;

#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
        if (mspMotorOverrideActive) {
            const bool reversibleMotorsEnabled = feature(FEATURE_REVERSIBLE_MOTORS);
            uint16_t overrideCommand = reversibleMotorsEnabled ? reversibleMotorsConfig()->neutral : motorConfig()->mincommand;
            uint16_t overrideUs;
            if (mspMotorOverrideGetValue(i, &overrideUs)) {
                overrideCommand = constrain(overrideUs, motorConfig()->mincommand, getMaxThrottle());
            }

            if (reversibleMotorsEnabled) {
                const reversibleMotorsConfig_t *revCfg = reversibleMotorsConfig();
                const reversibleMotorsThrottleState_e stateFromCommand = updateReversibleThrottleStateFromOverride(overrideCommand);
                if (stateFromCommand == MOTOR_DIRECTION_DEADBAND) {
                    motor[i] = revCfg->neutral;
                } else {
                    int mappedCommand = overrideCommand;
                    if (stateFromCommand == MOTOR_DIRECTION_BACKWARD) {
                        const int inverted = throttleRangeMin + (throttleRangeMax - overrideCommand);
                        mappedCommand = constrain(inverted, throttleRangeMin, throttleRangeMax);
                    }
                    motor[i] = mappedCommand;
                }
            } else {
                motor[i] = overrideCommand;
            }
        }
#endif
        ...
        if (isMotorProtocolDigital()) {
            ...
            if (feature(FEATURE_REVERSIBLE_MOTORS)) {
                if (reversibleMotorsThrottleState == MOTOR_DIRECTION_DEADBAND) {
                    motorValue = DSHOT_3D_DEADBAND_HIGH;
            ...
            if (feature(FEATURE_REVERSIBLE_MOTORS)) {
                if (reversibleMotorsThrottleState == MOTOR_DIRECTION_DEADBAND) {
                    motorValue = reversibleMotorsConfig()->neutral;
                } else if (reversibleMotorsThrottleState == MOTOR_DIRECTION_FORWARD) {
                    motorValue = handleOutputScaling(
                        motor[i],
                        throttleRangeMin,
...
static int getReversibleMotorsThrottleDeadband(void)
{
    ...
    } else if (reversibleMotorsThrottleState == MOTOR_DIRECTION_DEADBAND) {
        directionValue = reversibleMotorsConfig()->neutral;
    } else {
...
static reversibleMotorsThrottleState_e updateReversibleThrottleStateFromOverride(uint16_t overrideUs)
{
    ...
    if (overrideUs >= revCfg->deadband_high) {
        reversibleMotorsThrottleState = MOTOR_DIRECTION_FORWARD;
        throttleRangeMin = revCfg->deadband_high;
        throttleRangeMax = getMaxThrottle();
        motorValueWhenStopped = getReversibleMotorsThrottleDeadband();
        return reversibleMotorsThrottleState;
    }

    if (overrideUs <= revCfg->deadband_low) {
        reversibleMotorsThrottleState = MOTOR_DIRECTION_BACKWARD;
        throttleRangeMin = motorConfig()->mincommand;
        throttleRangeMax = revCfg->deadband_low;
        motorValueWhenStopped = getReversibleMotorsThrottleDeadband();
        return reversibleMotorsThrottleState;
    }

    reversibleMotorsThrottleState = MOTOR_DIRECTION_DEADBAND;
    throttleRangeMin = revCfg->deadband_low;
    throttleRangeMax = revCfg->deadband_high;
    motorValueWhenStopped = revCfg->neutral;
    return reversibleMotorsThrottleState;
}
#endif
