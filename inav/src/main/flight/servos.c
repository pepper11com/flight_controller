...
#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
        if (IS_RC_MODE_ACTIVE(BOXMSPRCOVERRIDE) && !failsafeIsActive() && mspServoOverrideIsFresh(micros())) {
            uint16_t overrideUs;
            // Map override payload entries to the sequential write order (servoIndex 0..N-1)
            if (mspServoOverrideGetValue((uint8_t)servoIndex, &overrideUs)) {
                servo[i] = constrain(overrideUs, servoParams(i)->min, servoParams(i)->max);
            }
        }
#endif
        if (zeroServoValue) {
