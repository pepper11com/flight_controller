...
#if defined(USE_RX_MSP) && defined(USE_MSP_RC_OVERRIDE)
        if (IS_RC_MODE_ACTIVE(BOXMSPRCOVERRIDE) && !failsafeIsActive() && mspMotorOverrideIsFresh(micros())) {
            uint16_t overrideUs;
            if (mspMotorOverrideGetValue(i, &overrideUs)) {
                motor[i] = constrain(overrideUs, motorConfig()->mincommand, getMaxThrottle());
            }
        }
#endif
